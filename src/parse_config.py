...
class ConfigParser:
    def __init__(self, config, resume=None, run_id=None, is_eval=False):
        """
        Class to parse configuration yaml file.
        ...
        _update_config(self.config, {"resume": resume})

        # set save_dir where trained model and log will be saved.
        save_dir = Path(self.config['experiment_config']['output_path'])

        exper_name = self.config['experiment_config']['experiment_name']
        
        if self.config['experiment_config']['experiment_id'] is None:
            run_id = datetime.now().strftime(r'%m%d_%H%M%S')
        else:
            run_id = self.config['experiment_config']['experiment_id']
        self.config['experiment_config']['experiment_id'] = run_id
        if is_eval:
            self._save_dir = save_dir / 'evaluation' / exper_name / run_id
    ...
    @classmethod
    def from_args(cls, args, options='', is_eval=False):
        """
        ...
        return cls(config, resume, modification, is_eval)
